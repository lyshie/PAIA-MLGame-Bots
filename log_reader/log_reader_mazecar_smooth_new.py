...
def main():
    ...
    for f in log_files:
        ...
        f = pd.DataFrame.from_dict(frames)
        for d in ['R', 'F', 'L']:
            f[d] = f[d + '_sensor'].apply(lambda x: x.get('distance'))
            f[d + '_x'] = f[d +
                            '_sensor'].apply(lambda x: x.get('coordinate')[0])
            f[d + '_y'] = f[d +
                            '_sensor'].apply(lambda x: x.get('coordinate')[1])
        f['end_x'] = f['end'].apply(lambda x: x[0])
        f['end_y'] = f['end'].apply(lambda x: x[1])
        f = f.drop(columns=['L_sensor', 'F_sensor', 'R_sensor', 'end'])
        c = pd.DataFrame.from_records([r[0] for r in p1_cmds[:-1]])
        c['direction'] = (c['left_PWM'].apply(lambda x: (np.sign(x) + 1) * 2) +
                          c['right_PWM'].apply(lambda x: (np.sign(x) + 1) * 1))
        c['direction2'] = c['direction'].apply(
            lambda x: ['B', '', '', 'N', 'L', 'R', 'F'][x])
        df = pd.concat([f, c], axis=1)
        print(df.to_string())

        #return

        ax.scatter3D(f['x'], f['y'], f['angle'], c=f['direction'])
        ...
        for a, b in zip(p1_cmds, frames):
            ...
            if a is not None:
                ...
                if b['F_sensor'] > 0:
                    smoother.warm(b['L_sensor'], b['F_sensor'], b['R_sensor'])
                else:
